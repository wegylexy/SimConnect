...
public sealed class SimConnect : IAsyncDisposable, IDisposable
{
    ...
    static readonly Regex IniSectionPattern = new(@"^\[SimConnect(?:\.(?<Index>\d+))?\]$(?:\n^(?:Protocol=(?<Protocol>Pipe|IPv4|IPv6)|Address=(?<Address>.+)|Port=(?<Port>.+)|MaxReceiveSize=(?<MaxReceiveSize>\d+)|DisableNagle=(?<DisableNagle>0|1)|(?!\[).*)$)*", RegexOptions.Multiline | RegexOptions.IgnoreCase);

    static async Task<Stream> ConnectClientStreamAsync(int index = default, CancellationToken cancellationToken = default)
    {
        string? address = null, port = null;
        try
        {
            var cfg = await File.ReadAllTextAsync("SimConnect.cfg", cancellationToken);
            var config = IniSectionPattern.Matches(cfg).Cast<Match>().Select(m => new
            {
                Index = m.Groups["Index"] is { Success: true, Value: var index } ? int.Parse(index) : default,
                Protocol = m.Groups["Protocol"] is { Success: true, Value: var protocol } ? protocol : null,
                Address = m.Groups["Address"] is { Success: true, Value: var address } ? address : null,
                Port = m.Groups["Port"] is { Success: true, Value: var port } ? port : null,
                MaxReceiveSize = m.Groups["MaxReceiveSize"] is { Success: true, Value: var receive } ? int.Parse(receive) : 8192,
                DisableNagle = m.Groups["DisableNagle"].Value == "1"
            }).FirstOrDefault(c => c.Index == index);
            if (config != null)
            {
                address = config.Address;
                port = config.Port;
                if (short.TryParse(port, out var p))
                {
                    TcpClient tc = new()
                    {
                        ReceiveBufferSize = config.MaxReceiveSize
                    };
                    await tc.ConnectAsync(address ?? "localhost", p, cancellationToken);
                    return tc.GetStream();
                }
            }
        }
        catch (FileNotFoundException) { }
        NamedPipeClientStream? stream = new(address ?? ".", port ?? @"Microsoft Flight Simulator\SimConnect", PipeDirection.InOut, PipeOptions.Asynchronous);
...
#pragma warning restore CS0612 // Type or member is obsolete
    ...
    public async ValueTask<RecvOpen> OpenAsync(string applicationName, CancellationToken cancellationToken = default)
    {
        ...
        {
            using (ClientStream) { }
            await ConnectClientStreamAsync(cancellationToken: lts.Token).ConfigureAwait(false);
    ...
    {
        if (typeof(T) is not { IsValueType: true, IsLayoutSequential: true, StructLayoutAttribute: { Pack: 1 } })
        {
    ...
    #region Client Data
    public async ValueTask<DataDefinition> DefineClientDataAsync<T>(CancellationToken cancellationToken = default) where T : unmanaged
    {
        if (typeof(T) is not { IsValueType: true, IsLayoutSequential: true, StructLayoutAttribute: { Pack: 1 } })
        {
            throw new NotSupportedException(); // TODO: support when requested in tagged format
        }
        var size = Unsafe.SizeOf<SendAddToClientDataDefinition>();
        var a = ArrayPool<byte>.Shared.Rent(size);
        try
        {
            var m = a.AsMemory(0, size);
            ...
            foreach (var f in fields)
            {
                var d = f.GetCustomAttribute<ClientDataDefinitionAttribute>() ?? new();
                unsafe
                {
                    var (t, s) = f.FieldType.Name switch
                    {
                        nameof(SByte) => (SendAddToClientDataDefinition.Int8, sizeof(sbyte)),
                        nameof(Int16) => (SendAddToClientDataDefinition.Int16, sizeof(short)),
                        nameof(Int32) => (SendAddToClientDataDefinition.Int32, sizeof(int)),
                        nameof(Int64) => (SendAddToClientDataDefinition.Int64, sizeof(long)),
                        nameof(Single) => (SendAddToClientDataDefinition.Float32, sizeof(float)),
                        nameof(Double) => (SendAddToClientDataDefinition.Float64, sizeof(double)),
                        _ => typeof(Unsafe).GetMethod(nameof(Unsafe.SizeOf), 1, Type.EmptyTypes)?
                            .MakeGenericMethod(f.FieldType).Invoke(null, null) is int i ? (i, i) :
                            throw new NotSupportedException()
                    };
                    used[d.DatumId = datumId] = (offset, s);
                    offset += s;
                    MemoryMarshal.AsRef<SendAddToClientDataDefinition>(m.Span) =
                        new(id, (uint)offset, t, d.Epsilon, d.DatumId);
                }
                adding[datumId] = (await WriteAsync(m, cancellationToken).ConfigureAwait(false)).ContinueWith(task =>
                {
                    if (task.Exception?.InnerException is AsyncException ee)
            ...
            return _defines[typeof(T)] = new(id, used, Task.WhenAll(adding), async () =>
            {
                if (_defines.TryRemove(typeof(T), out var d))
                {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            MemoryMarshal.AsRef<SendClearClientDataDefinition>(a) = new(d.DefineId);
                            _ = await WriteAsync(a.AsMemory(0, size), cancellationToken).ConfigureAwait(false);
                        }
                        finally
                        {
