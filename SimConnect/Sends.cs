...
readonly struct SendAddToDataDefinition
{
    ...
    public unsafe SendAddToDataDefinition(uint defineId, string datumName, string? unitsName,
        DataType datumType = DataType.Float64, float epsilon = default, int datumId = -1) =>
...
readonly struct SendRequestDataOnSimObject
{
    ...
    public unsafe SendRequestDataOnSimObject(uint requestId, uint defineId, uint objectId, Period period, DataRequestFlags flags = DataRequestFlags.Default, uint origin = default, uint interval = default, uint limit = default) =>
...
readonly struct SendMapInputEventToClientEvent
{
    ...
    public unsafe SendMapInputEventToClientEvent(uint groupId, string inputDefinition, uint downEventId, int downValue = default, uint upEventId = uint.MaxValue, int upValue = default, bool maskable = false) =>
...
#region Client Data
[StructLayout(LayoutKind.Sequential, Pack = 1)]
readonly struct SendMapClientDataNameToId
{
    readonly Send Send;
    readonly String256 Name;
    readonly uint ClientDataId;

    public unsafe SendMapClientDataNameToId(string name, uint clientDataId) =>
        (Send, Name, ClientDataId) =
            (new(sizeof(SendMapClientDataNameToId), 0xF0000037), name, clientDataId);
}

[StructLayout(LayoutKind.Sequential, Pack = 1)]
readonly struct SendCreateClientData
{
    public const uint MaxSize = 8192;

    readonly Send Send;
    readonly uint ClientDataId, Size;
    readonly ClientDataFlags Flags;

    public unsafe SendCreateClientData(uint clientDataId, uint size, ClientDataFlags flags) =>
        (Send, ClientDataId, Size, Flags) =
            (new(sizeof(SendCreateClientData), 0xF0000038), clientDataId, size, flags);
}

[StructLayout(LayoutKind.Sequential, Pack = 1)]
readonly struct SendAddToClientDataDefinition
{
    public const int Int8 = -1, Int16 = -2, Int32 = -3, Int64 = -4, Float32 = -5, Float64 = -6;

    readonly Send Send;
    readonly uint DefineId, Offset;
    readonly int SizeOrType;
    readonly float Epsilon;
    readonly int DatumId;

    public unsafe SendAddToClientDataDefinition(uint defineId, uint offset, int sizeOrType, float epsilon = default, int datumId = -1) =>
        (Send, DefineId, Offset, SizeOrType, Epsilon, DatumId) =
            (new(sizeof(SendAddToClientDataDefinition), 0xF0000039), defineId, offset, sizeOrType, epsilon, datumId);
}

[StructLayout(LayoutKind.Sequential, Pack = 1)]
readonly struct SendClearClientDataDefinition
{
    readonly Send Send;
    readonly uint DefineId;

    public unsafe SendClearClientDataDefinition(uint defineId) =>
        (Send, DefineId) = (new(sizeof(SendClearClientDataDefinition), 0xF000003A), defineId);
}

[StructLayout(LayoutKind.Sequential, Pack = 1)]
readonly struct SendRequestClientData
{
    readonly Send Send;
    readonly uint ClientDataId, RequestId, DefineId;
    readonly Period Period;
    readonly ClientDataRequestFlags Flags;
    readonly uint Origin, Interval, Limit;

    public unsafe SendRequestClientData(uint clientDataId, uint requestId, uint defineId, Period period = Period.Once, ClientDataRequestFlags flags = ClientDataRequestFlags.Default, uint origin = default, uint interval = default, uint limit = default) =>
        (Send, ClientDataId, RequestId, DefineId, Period, Flags, Origin, Interval, Limit) =
            (new(sizeof(SendRequestClientData), 0xF000003B), clientDataId, requestId, defineId, period, flags, origin, interval, limit);
}
#endregion